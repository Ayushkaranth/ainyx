...
export const ServiceNode = memo(({ data, selected }: NodeProps<any>) => {
  const typedData = data as AppNodeData;
  
  const Icon = ICONS[typedData.type] || Server;
  const [activeTab, setActiveTab] = useState<MetricTab>('CPU');
  
  const isSuccess = typedData.status === 'Success';
    ...
    <Card className={cn("w-[340px] border-2 shadow-xl transition-all duration-200 bg-card", selected ? 'border-primary' : 'border-border')}>
      ...
      <CardHeader className="p-4 pb-0">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-lg font-bold">{typedData.label}</span>
          </div>
           <div className="flex items-center gap-2">
             <Badge variant="outline" className="text-green-400 border-green-400/30 font-mono">{typedData.cost}</Badge>
      ...
      <CardContent className="p-4 space-y-4">
        <div className="flex justify-between gap-2 bg-background/50 p-1 rounded-lg">
           <TabButton tab="CPU" icon={Cpu} activeTab={activeTab} setActiveTab={setActiveTab} value={typedData.specs.cpu} />
           <TabButton tab="Memory" icon={MemoryStick} activeTab={activeTab} setActiveTab={setActiveTab} value={typedData.specs.memory} />
           <TabButton tab="Disk" icon={HardDrive} activeTab={activeTab} setActiveTab={setActiveTab} value={typedData.specs.disk} />
        ...
        <div className="space-y-2">
           <div className="flex justify-end">
              <span className="font-mono text-sm">{typedData.traffic.toFixed(2)}</span>
           </div>
          <div className="h-2 w-full bg-secondary rounded-full overflow-hidden relative">
             <div className={cn("absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-primary shadow-sm")} style={{ left: `calc(${typedData.traffic}% - 6px)` }} />
            <div className={cn("h-full transition-all duration-300 ease-out", activeTab === 'CPU' ? 'bg-blue-500' : activeTab === 'Memory' ? 'bg-purple-500' : 'bg-orange-500')} style={{ width: `${typedData.traffic}%` }} />
          </div>
        </div>
        ...
        <div className="flex justify-between items-end pt-2">
           <Badge variant="outline" className={cn("flex items-center gap-1 pl-1 pr-2 py-0.5 font-medium transition-colors", statusBg, statusColor)}>
              <StatusIcon className="w-3.5 h-3.5" /> {typedData.status}
           </Badge>
